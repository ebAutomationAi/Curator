# Curator — Operations Guide

Guía operativa de despliegue, mantenimiento, backup y restauración para Curator. Para una descripción general del proyecto, ver [README.md](README.md).
...
## Monitorización con Healthchecks.io (opcional)

El contenedor `curator` expone `GET /health` y su `HEALTHCHECK` de Docker lo comprueba cada 20 minutos. Si además defines `HEALTHCHECK_UUID` en `.env` con el UUID de un check de [Healthchecks.io](https://healthchecks.io), cada comprobación exitosa hace también un ping a `https://hc-ping.com/$HEALTHCHECK_UUID` — así recibes una alerta externa (email/push) si el servicio deja de responder.

```bash
# .env
HEALTHCHECK_UUID=tu-uuid-de-healthchecks-io
```

Tras añadirlo, recrea el contenedor para que recoja la variable:
```bash
docker compose up -d --force-recreate curator
```

Si se deja vacío, el healthcheck sigue funcionando solo con la comprobación local (sin ping externo).
